...
app.get('/api/orkut/createpayment', async (req, res) => {
    ...
    if (!apikey) {
    return res.json("Apikey Wajib Di Isi.");
    }
...
app.get("/api/ai/openai-prompt", async (req, res) => {
    ...
    if (!prompt || !msg) return res.json("Isi Parameter nya bang !!!!");
...
app.get("/api/ai/openai", async (req, res) => {
    const { msg } = req.query;
    if (!msg) return res.json("Isi Parameter nya bang !!!!");
...
app.get("/api/ai/gpt4", async (req, res) => {
    ...
    if (!text) return res.json("Isi Parameter nya bang !!!!");
...
app.get("/api/download/fbdl", async (req, res) => {
    const { url } = req.query;
    if (!url) return res.json("Isi Parameter nya bang !!!!");
...
app.get("/api/tools/tiktokstalk", async (req, res) => {
    try {     
      ...
      if (!user) return res.json("Isi Parameter nya bang !!!!");
...
if (!egg || !nestid || !loc || !domain || !ptla || !ptlc) return res.json("Isi Parameter nya bang !!!!")
domain = "https://" + domain
let listnya = []
...
app.get("/api/search/pinterest", async (req, res) => {
    try {     
      const { q } = req.query
      if (!q) return res.json("Isi Parameter nya bang !!!!");
...
app.get("/api/imagecreator/pornhub", async (req, res) => {
    try {     
      ...
      if (!text1 || !text2) return res.json("Isi Parameter nya bang !!!!");
...
app.get("/api/imagecreator/qc", async (req, res) => {
    try {     
      ...
      if (!text || !nama || !fotoUrl) return res.json("Isi Parameter nya bang !!!!");
const json = {
  "type": "quote",
  "format": "png",
